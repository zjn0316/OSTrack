...
- `MODEL.PRETRAIN_FILE` 在 Stage-2 测试/训练时应指向已训练的 OSTrack 视觉 checkpoint，例如 `output/checkpoints/train/ostrack/vitb_256_mae_32x4_ep300/OSTrack_ep0300.pth.tar`。若测试时只加载 UWB-only checkpoint 而不加载视觉 tracker 权重，预测框会快速漂移，后续 `sample_target()` 的 search crop 会和原版 OSTrack 不一致。
